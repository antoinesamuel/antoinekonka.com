...
export default function Header({ logoSrc }: HeaderProps) {
  ...
  return (
    <>
      {/* ========================================== */}
      <header className="hidden md:flex absolute top-0 left-0 w-full z-30 justify-between items-center px-12 py-6 pointer-events-auto">
        ...
        <nav className="flex items-center space-x-1 px-6 py-4 rounded-full border border-white/15 bg-white/5 backdrop-blur-xl shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]">
          <Link
            href="/About"
            className="px-5 py-2 text-white/80 hover:text-white transition-colors text-[1.75rem] font-medium rounded-full"
          >
            About
          </Link>
          <Link
